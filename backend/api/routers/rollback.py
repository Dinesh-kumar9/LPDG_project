...
@router.get("/log")
def get_log(settings: Settings = Depends(get_settings)) -> dict[str, Any]:
...
@router.post("/execute")
def execute_rollback(body: RollbackBody, settings: Settings = Depends(get_settings)) -> dict[str, Any]:
    try:
        ...
        if settings.data_dir.exists():
            predict(data_dir=settings.data_dir, out_path=settings.predictions_path, models_dir=settings.models_dir)
        ...
        raise HTTPException(status_code=500, detail=f"Rollback execution failed: {e}")
...
def verify_version(
    ...
    try:
        passed = verify(data_dir=settings.data_dir, version_id=active, models_dir=settings.models_dir)
        return {
            "version_id": active,
            "verified": passed,
        ...
        raise HTTPException(status_code=500, detail=f"Verification failed with error: {e}")
